...
DISABLED_CHANNELS=$(cat larpix-10x10-scripts/disabled_channels_cold_10x10chips.json)
...
python3.6 -i autoconfig_thresholds.py --controller_config controller/$CONTROLLER_CONFIG --runtime $RUNTIME --disabled_channels "$DISABLED_CHANNELS" #--chip_key $CHIP_KEY #--target_rate $TARGET_RATE #--disable_threshold $DISABLE_THRESHOLD #--channels $CHANNELS 

##mv $(find . -name 'config-*.json') $WORKDIR/DAQ/thresholds/
##cd $WORKDIR

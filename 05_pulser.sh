...
DATADIR='/data/LArPix'
THRESHOLDFILES=$DATADIR'/'$EXPERIMENT'/thresholdFiles/' # Config file or directory to load chip configs from (if given a folder, it will take the last produced config files)
...
echo Using thresholds from $THRESHOLDFILE

cd $WORKDIR/larpix-10x10-scripts
python3 -i internal_pulse.py --config_name $THRESHOLDFILES \
...
mv $DATAFILE $DATADIR/$EXPERIMENT/pulserRuns/$DATAFILE
echo 'Pulser file saved in' $DATADIR'/'$EXPERIMENT'/pulserRuns/'$DATAFILE
...
# python3 -i plot_testpulse.py 1000000 $WORKDIR/larpix-v2-testing-scripts/plotting-scripts/pulser_config_file.json

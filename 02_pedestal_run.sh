...
RUNTIME=60 # 120                # Duration for run (in seconds) (default=60)
CHANNELS='[0,1]'                # if used, uncomment below
CHIP_KEY='1-1-11'               # if used, uncomment below
DISABLED_CHANNELS=$(cat /home/lhep/PACMAN/larpix-10x10-scripts/disabled_channels_warm_10x10chips_2020_11_20.json) # if used, uncomment below
#############################################################################

#WORKDIR="$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")"
WORKDIR='/home/lhep/PACMAN'

echo "$DISABLED_CHANNELS"
...
python3.6 -i $WORKDIR/larpix-v2-testing-scripts/plotting-scripts/plot_pedestal.py $WORKDIR/DAQ/$EXPERIMENT/pedestalRuns/$DATAFILE

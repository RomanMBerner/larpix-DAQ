...
python3.6 leakage_current_rough.py --controller_config controller/$CONTROLLER_CONFIG \
                                      --threshold $THRESHOLD \
                                      --runtime $RUNTIME
                                      #--channels $CHANNELS
                                      #--chip_key $CHIP_KEY
...
python3.6 $WORKDIR/larpix-v2-testing-scripts/plotting-scripts/plot_leakage.py $WORKDIR/DAQ/$EXPERIMENT/leakageCurrent/$DATAFILE

...
python3.6 -i leakage_current_rough.py --controller_config controller/$CONTROLLER_CONFIG --threshold $THRESHOLD --runtime $RUNTIME #--channels $CHANNELS --chip_key $CHIP_KEY

##unset -v DATAFILE
##DATAFILE=$(ls -tr datalog* | tail -n 1)
##mv $DATAFILE $WORKDIR/DAQ/leakageCurrent/$DATAFILE
##echo "Leakage run file saved in $WORKDIR/DAQ/leakageCurrents/$DATAFILE"
##cd $WORKDIR/DAQ/leakageCurrent/
##python2.7 -i $WORKDIR/larpix-v2-testing-scripts/plotting-scripts/plot_leakage.py $WORKDIR/DAQ/leakageCurrent/$DATAFILE
##cd $WORKDIR

...
if ! $LOGGER && ! $NO_RESET
then
    python3.6 -i base.py --controller_config controller/$CONTROLLER_CONFIG
fi
if $LOGGER && ! $NO_RESET
then
    python3.6 -i base.py --controller_config controller/$CONTROLLER_CONFIG --logger
fi
if ! $LOGGER && $NO_RESET
then
    python3.6 -i base.py --controller_config controller/$CONTROLLER_CONFIG --no_reset
fi
if $LOGGER && $NO_RESET
then
    python3.6 -i base.py --controller_config controller/$CONTROLLER_CONFIG --logger --no_reset
fi
